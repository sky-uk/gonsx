...
func TestGetAllUnMarshalling(t *testing.T) {
	...
	assert.Nil(t, xmlErr)
	assert.Len(t, getAllIpSetAPI.GetResponse().IpSets, 3)
	assert.Equal(t, "OVP_sg2", getAllIpSetAPI.GetResponse().IpSets[0].Name)
	// bit deep level checks to test that we un-marshalling the whole object.
	assert.Len(t, getAllIpSetAPI.GetResponse().IpSets[0].DynamicMemberDefinition.DynamicSet, 2)
	assert.Equal(t, "OR", getAllIpSetAPI.GetResponse().IpSets[0].DynamicMemberDefinition.DynamicSet[0].Operator)
	assert.Equal(t, "app4", getAllIpSetAPI.GetResponse().IpSets[0].DynamicMemberDefinition.DynamicSet[0].DynamicCriteria[0].Value)
}
