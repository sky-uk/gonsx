...
func NewDelete(deleteSection Section) *DeleteFWSectionAPI {
	...
	case "LAYER3":
		endpoint = fmt.Sprintf("/api/4.0/firewall/globalroot-0/config/layer3sections/%d", deleteSection.ID)
	case "LAYER2":
		endpoint = fmt.Sprintf("/api/4.0/firewall/globalroot-0/config/layer2sections/%d", deleteSection.ID)
	}
